        ...
        private void InitStandardClassSource()
        {
            ...
            if (dtClass.Rows.Count > 0)
            {
                ...
                for (int i = 0; i < dtClass.Rows.Count; i++)
                {
                    ...
                    double startCloseOfNewCount = 0, startCloseOfNewWeight = 0, startCloseOfNewAmount = 0;
                    ...
                    double endOverdueCount = 0, endOverdueWeight = 0, endOverdueAmount = 0;
                    double endCount = 0, endWeight = 0, endAmount = 0;
                    double endNewCount = 0, endNewWeight = 0, endNewAmount = 0;
                    double endCloseOfNewCount = 0, endCloseOfNewWeight = 0, endCloseOfNewAmount = 0;
                            ...
                            if (dtOperations.Rows.Count > 0)
                            {
                                for (int k = 0; k < dtOperations.Rows.Count; k++)
                                {
                                    ...
                                    if (startValue < 0)   //operateDate<startDate,期初
                                    {
                                        ...
                                        switch (operationType)
                                        {
                                            case 1:
                                                ...
                                                {
                                                    startCloseOfNewCount += Convert.ToDouble(dtPawnageInfo.Rows[j]["CountNumber"]);
                                                    startCloseOfNewWeight += Convert.ToDouble(dtPawnageInfo.Rows[j]["Weight"]);
                                                    startCloseOfNewAmount += Convert.ToDouble(dtOperations.Rows[k]["Amount"]);
                                                }
                                                break;
                                            //case 2:
                                            //    startCount += Convert.ToDouble(dtPawnageInfo.Rows[j]["CountNumber"]);
                                            //    startWeight += Convert.ToDouble(dtPawnageInfo.Rows[j]["Weight"]);
                                            //    startAmount += Convert.ToDouble(dtOperations.Rows[k]["Amount"]);
                                            //    break;
                                            case 4:
                                    ...
                                    if (endValue <= 0)   //期末
                                    {
                                        ...
                                        {
                                            case 1:
                                                endNewCount += Convert.ToDouble(dtPawnageInfo.Rows[j]["CountNumber"]);
                                                endNewWeight += Convert.ToDouble(dtPawnageInfo.Rows[j]["Weight"]);
                                                endNewAmount += Convert.ToDouble(dtOperations.Rows[k]["Amount"]);
                                                if (newTicket.StatusID == 4 || newTicket.StatusID == 7)
                                                {
                                                    endCloseOfNewCount += Convert.ToDouble(dtPawnageInfo.Rows[j]["CountNumber"]);
                                                    endCloseOfNewWeight += Convert.ToDouble(dtPawnageInfo.Rows[j]["Weight"]);
                                                    endCloseOfNewAmount += Convert.ToDouble(dtOperations.Rows[k]["Amount"]);
                                                }
                                                if (intOverdue >= 0)   //期末在期
                                                {
                                                    endInScheduleCount += Convert.ToDouble(dtPawnageInfo.Rows[j]["CountNumber"]);
                                            ...
                                            case 2:
                                                endCount += Convert.ToDouble(dtPawnageInfo.Rows[j]["CountNumber"]);
                                                endWeight += Convert.ToDouble(dtPawnageInfo.Rows[j]["Weight"]);
                                                endAmount += Convert.ToDouble(dtOperations.Rows[k]["Amount"]);
                                                if (intOverdue >= 0)   //期末在期
                                                {
                                                    endInScheduleCount += Convert.ToDouble(dtPawnageInfo.Rows[j]["CountNumber"]);
                                                    endInScheduleWeight += Convert.ToDouble(dtPawnageInfo.Rows[j]["Weight"]);
                    ...
                    startCount = startNewCount - startCloseOfNewCount;
                    startWeight = startNewWeight - startCloseOfNewWeight;
                    startAmount = startNewAmount - startCloseOfNewAmount;
                    drow["StartCount"] = startCount;
                    drow["StartWeight"] = startWeight;
                    drow["StartAmount"] = startAmount;
                    ...
                    endAmount = endNewAmount - endCloseOfNewAmount;
                    endCount = endNewCount - endCloseOfNewCount;
                    endWeight = endNewWeight - endCloseOfNewWeight;
                    drow["EndCount"] = endCount;
                    drow["EndWeight"] = endWeight;
                    drow["EndAmount"] = endAmount;
